...
def read_friendship_network(filename):
    ...
    #variable network dictionary initialized
    network = {}
    ...
    try:
        ...
        for line in file:
            ...
            if len(line) > 0 and line[0] == "#":
                continue
            #gives a list of network names eg.['Greta', 'Helen', 'Irina', 'Florian']
            fields = line.split(";")

            #if it has one name only error
            if len(fields) < 2:
                print(f"Fatal Error: line '{line}' has too few fields.")
                return None

            #name = Greta , condition makes sure name is from a-z
            for name in fields:
                if not name.isalpha():
...
def add_friendship(network, name1, name2):
    ...
    #       already known to be friends, nothing needs to be done.
        ...
        #adding name1 and name 2 to the dict Network
        #by creating two lists in the dictionary with a conditional statement
        # else::: if name1 and name 2 are already in the network do nothing
        ...
        #if (name1,name2) in network.items():
            #return

        #if if name1 and name2 are not in nested lists as values
        ...
        if name2 not in network[name1]:
            ...
            #for line in network[name1]:
                #if line[0] in network:
...
def print_command(network, namelist):
    ...
    for key_names in sorted(network):
        print(key_names) #displays all names in network dict keys_names=keys
        for friends in sorted(network[key_names]):
            print(f"- {friends}") #loop and print the values of key words in the dict
...
def friends_command(network, namelist):
    ...
    for friends in sorted(network[name]):
        print(friends)
...
def common_friends_command(network, namelist):
    ...
    #same name/person cannot be freinds
    if name1== name2:
    ...
    #create list to store common friends
    common_friends = []
    ...
    #if <name1> and <name2> have no common friend
    if len(common_friends)== 0: #no common
          print(f"{name1} and {name2} have no common friends.")

    #display all common friends in alphabetical order
    for friend_names in sorted(common_friends):
        print(friend_names)
