...
class Car:
    ...
    def drive(self,distance):
        ...
        if total_km - distance <0:
            self.__gas = 0.0
            self.__odometer = 100.0
            return self.__odometer, self.__gas


        else:
            self.__gas = (total_km - distance)/ self.__gas

            self.__odometer += distance

            if self.__gas > self.__tank_volume or self.__odometer > 100:
                self.__gas = 0.0
                self.__odometer = total_km
                return self.__odometer, self.__gas
